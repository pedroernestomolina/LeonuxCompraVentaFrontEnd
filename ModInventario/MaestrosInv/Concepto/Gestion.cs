...
namespace ModInventario.MaestrosInv.Concepto
{
    ...
    public class Gestion: IMaestroTipo
    {
        ...
        public string Titulo { get { return "MAESTRO: CONCEPTOS INVENTARIO"; } }
        ...
        public void AgregarItem()
        {
            ...
            if (_gSeguridad.Verificar(r00.Entidad))
            {
                ...
                if (_gAgregar.IsOk)
                {
                    var r01 = Sistema.MyData.Concepto_GetFicha(_gAgregar.ItemAgregarId);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError)
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
        ...
        public void EditarItem(data ItemActual)
        {
            ...
            if (_gSeguridad.Verificar(r00.Entidad))
            {
                ...
                if (_gEditar.IsOk)
                {
                    var r01 = Sistema.MyData.Concepto_GetFicha(idEditar);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError)
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
        ...
        public void EliminarItem(data ItemActual)
        {
            ...
            if (_gSeguridad.Verificar(r00.Entidad))
            {
                ...
                {
                    var r01 = Sistema.MyData.Concepto_Eliminar(ItemActual.auto);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError)
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
