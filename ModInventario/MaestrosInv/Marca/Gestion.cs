...
namespace ModInventario.MaestrosInv.Marca
{
    ...
    public class Gestion: IMaestroTipo
    {
        ...
        public string Titulo { get { return "Maestro: MARCAS"; } }
        ...
        public void AgregarItem()
        {
            ...
            if (_gAcceso.Verificar(r00.Entidad))
            {
                ...
                if (_gAgregar.IsOk)
                {
                    var r01 = Sistema.MyData.Marca_GetFicha(_gAgregar.IdItemAgregado);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError) 
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
        ...
        {
            _eliminarIsOk = false;
            var r00 = Sistema.MyData.Permiso_EliminarMarca(Sistema.UsuarioP.autoGru);
            if (r00.Result == OOB.Enumerados.EnumResult.isError)
            {
                Helpers.Msg.Error(r00.Mensaje);
                return;
            ...
            if (_gAcceso.Verificar(r00.Entidad))
            {
                ...
                {
                    var r01 = Sistema.MyData.Marca_Eliminar(id);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError)
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
