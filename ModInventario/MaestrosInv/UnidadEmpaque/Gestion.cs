...
namespace ModInventario.MaestrosInv.UnidadEmpaque
{
    ...
    public class Gestion: IMaestroTipo
    {
        ...
        public string Titulo { get { return "Maestro: EMPAQUE-MEDIDA"; } }
        ...
        public void AgregarItem()
        {
            ...
            if (_gAcceso.Verificar(r00.Entidad))
            {
                ...
                if (_gAgregar.IsOk)
                {
                    var r01 = Sistema.MyData.EmpaqueMedida_GetFicha(_gAgregar.IdItemAgregado);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError) 
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
                    }
                    _itemAgregarEditar = new data(r01.Entidad.auto, "", r01.Entidad.nombre);
                }
            }
        }
        ...
        public void EditarItem(string id)
        {
            ...
            if (_gAcceso.Verificar(r00.Entidad))
            {
                ...
                if (_gEditar.IsOk)
                {
                    var r01 = Sistema.MyData.EmpaqueMedida_GetFicha(idItemEditar);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError)
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
        ...
        {
            _eliminarIsOk = false;
            var r00 = Sistema.MyData.Permiso_EliminarUnidadEmpaque(Sistema.UsuarioP.autoGru);
            if (r00.Result == OOB.Enumerados.EnumResult.isError)
            {
                Helpers.Msg.Error(r00.Mensaje);
                return;
            ...
            if (_gAcceso.Verificar(r00.Entidad))
            {
                ...
                {
                    var r01 = Sistema.MyData.EmpaqueMedida_Eliminar(id);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError)
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
