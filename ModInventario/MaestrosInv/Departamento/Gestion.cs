    ...
    public class Gestion: IMaestroTipo
    {
        ...
        public void AgregarItem()
        {
            ...
            if (_gAcceso.Verificar(r00.Entidad))
            {
                ...
                if (_gAgregar.IsOk)
                {
                    var r01 = Sistema.MyData.Departamento_GetFicha(_gAgregar.IdItemAgregado);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError) 
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
        ...
        public void EditarItem(string id)
        {
            ...
            if (_gAcceso.Verificar(r00.Entidad))
            {
                ...
                if (_gEditar.IsOk)
                {
                    var r01 = Sistema.MyData.Departamento_GetFicha(idItemEditar);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError)
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
        ...
        public void EliminarItem(string id)
        {
            ...
            if (_gAcceso.Verificar(r00.Entidad))
            {
                ...
                {
                    var r01 = Sistema.MyData.Departamento_Eliminar(id);
                    if (r01.Result == OOB.Enumerados.EnumResult.isError)
                    {
                        Helpers.Msg.Error(r01.Mensaje);
                        return;
